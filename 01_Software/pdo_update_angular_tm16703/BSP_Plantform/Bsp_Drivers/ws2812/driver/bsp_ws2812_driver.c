...
/********************************** Functions ********************************/
static void ws2811_store_color(uint8_t index, uint8_t r, uint8_t g, uint8_t b)
{
...
#elif (WS2811_COLOR_ORDER == WS2811_COLOR_ORDER_RBG)
    ws2811_colors[index][0] = r;
    ws2811_colors[index][1] = b;
    ws2811_colors[index][2] = g;
#elif (WS2811_COLOR_ORDER == WS2811_COLOR_ORDER_GRB)
    ws2811_colors[index][0] = g;
    ws2811_colors[index][1] = r;
    ws2811_colors[index][2] = b;
#elif (WS2811_COLOR_ORDER == WS2811_COLOR_ORDER_GBR)
    ws2811_colors[index][0] = g;
    ws2811_colors[index][1] = b;
    ws2811_colors[index][2] = r;
#elif (WS2811_COLOR_ORDER == WS2811_COLOR_ORDER_BRG)
    ws2811_colors[index][0] = b;
    ws2811_colors[index][1] = r;
    ws2811_colors[index][2] = g;
#elif (WS2811_COLOR_ORDER == WS2811_COLOR_ORDER_BGR)
    ws2811_colors[index][0] = b;
    ws2811_colors[index][1] = g;
    ws2811_colors[index][2] = r;
#else
#error "Unsupported WS2811_COLOR_ORDER"
#endif
}
...
static void ws2811_encode(const uint8_t *rgb, uint32_t len)
{
    ...
    memset(&ws2811_buf[k], 0, (WS2811_BUF_LEN - k) * sizeof(ws2811_buf[0]));
}
...
void ws2811_show(void)
{
	...
	(void)pwm_port_start_dma(PWM_DEV_WS2812,
	                         ws2811_buf,
	                         (uint16_t)(sizeof(ws2811_buf) / sizeof(ws2811_buf[0])));
}
...
void ws2811_set_pixel(const uint8_t index, const uint32_t rgb)
{
    ...
    ws2811_store_color(index, r, g, b);
}
...
void ws2811_fade(const uint8_t start_index, const uint8_t end_index, const uint32_t rgb)
{
    ...
    {
      ws2811_store_color((uint8_t)i, r_out, g_out, b_out);
    }
                 ...
                 const uint8_t dir, const uint32_t rgb)
{
    ...
    uint8_t b = (rgb)       & 0xFF;
		...
		ws2811_store_color((uint8_t)cur_index, r, g, b);
