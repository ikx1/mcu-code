...
static void telemetry_service_can_diag_clear_sticky_window(const can_irq_diag_t *can_diag,
                                                           uint32_t can_rx_drop,
                                                           TickType_t now)
{
    if (can_diag == NULL)
    ...
    /* Rebase the sticky window to the latest observed counters instead of
     * clearing the producers, so any fault that arrives after this snapshot
     * remains visible on the next maintenance cycle. */
    s_telemetry_can_diag.last_rx_drop_count = can_rx_drop;
    s_telemetry_can_diag.last_busoff_count = can_diag->busoff_count;
    s_telemetry_can_diag.last_recover_fail_count = can_diag->recover_fail_count;
    s_telemetry_can_diag.last_change_tick = now;
...
void telemetry_service_diag_maintenance(void)
{
    ...
    if (need_clear != 0u)
    {
        telemetry_service_can_diag_clear_sticky_window(&can_diag,
                                                       can_rx_drop,
                                                       now);
    }
    taskEXIT_CRITICAL();
