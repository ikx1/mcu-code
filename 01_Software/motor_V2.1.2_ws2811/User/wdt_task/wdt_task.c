...
static wdt_monitor_data_t wdt_data = {
    .inited = false,
    .lock = NULL,
    .thread = NULL,
    .task_num = 0,
...
/********************************** Functions ********************************/
static uint32_t wdt_ms_to_ticks(uint32_t period_ms)
{
                                     ...
                                     uint32_t *out_id)
{
    ...
    /************* check param  **************************/
    if ((NULL == name) || (NULL == out_id) || (max_period == 0u))
    {
    ...
    if (wdt_data.task_num >= WDT_MONITOR_TASK_MAX)
    {
        xSemaphoreGive(wdt_data.lock);
        return WDT_ERROR_RESOURCES;
    }

    /************* register thread monitor ***************/
    wdt_data.tasks[wdt_data.task_num].name = name;
    wdt_data.tasks[wdt_data.task_num].max_period_tick = wdt_ms_to_ticks(max_period);
    wdt_data.tasks[wdt_data.task_num].enable = false;
    wdt_data.tasks[wdt_data.task_num].last_update = (uint32_t)WDT_TICK;
...
ret_code_t wdt_monitor_task_enable(uint32_t id)
{
    ...
    wdt_data.tasks[id].last_update = (uint32_t)WDT_TICK;
...
ret_code_t wdt_monitor_task_period_set(uint32_t id, uint32_t max_period)
{
    ...
    /************* check param **************************/
    if ((id >= WDT_MONITOR_TASK_MAX) || (max_period == 0u))
    {
        // NRF_LOG_INFO(" [%s] invalid param !!! \r\n", __FUNCTION__);
        return WDT_ERROR_INVALID_PARAM;
    }
    ...
    /************* register thread monitor ***************/
    wdt_data.tasks[id].max_period_tick = wdt_ms_to_ticks(max_period);
...
void wdt_monitor_thread(void *arg)
{
    ...
    taskENTER_CRITICAL();
    wdt_data.thread = xTaskGetCurrentTaskHandle();
    taskEXIT_CRITICAL();

    TickType_t xTicksToWait = pdMS_TO_TICKS(WDT_MONITOR_WAIT_MS);
    if (wdt_monitor_init() != WDT_SUCCESS)
    {
        system_hang_up();
    }
