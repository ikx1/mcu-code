...
/*
帧格式：
[0]=H1 [1]=H2 [2]=VER [3]=SEQ [4]=TYPE [5]=LEN_L [6]=LEN_H [7..]=PAYLOAD [end-2]=CRC_L [end-1]=CRC_H
CRC over VER..PAYLOAD => starting at [2], length = 5 + payload_len
*/
bool uart_parser_feed(uart_parser_t *p, uint8_t byte, uart_frame_t *out_frame)
{
    ...
    case ST_FIND_H2:
        if (byte == H2) {
            p->buf[p->pos++] = byte;
            p->need = 2 + 5; /* H1H2 + (VER SEQ TYPE LEN_L LEN_H) */
            p->st = ST_READ_HDR;
        } else if (byte == H1) {
            p->buf[0] = byte;
            p->pos = 1;
    ...
    case ST_READ_HDR:
        ...
        if (p->pos >= p->need)
        {
            ...
            p->payload_len = len;
            p->need = (uint16_t)(2 + 5 + len + 2); /* H1H2 + hdr5 + payload + crc2 */
...
{
    if (!b || !b->out || b->max < 7u) return false;
    if (b->payload_start < 7u || b->payload_start > b->w) return false;
    if (b->w > b->max) return false;
    uint16_t payload_len = (uint16_t)(b->w - b->payload_start);
    if (payload_len > UART_MAX_PAYLOAD) return false;

    wr_le16(&b->out[5], payload_len); /* LEN at [5..6] */

    uint16_t crc = crc16_modbus(&b->out[2], (uint16_t)(5 + payload_len));

    if (b->w + 2 > b->max) return false;
